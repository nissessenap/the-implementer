...
# clone <repo> — through the proxy, with the sentinel the proxy swaps. The same
# URL shape the run plan builds, because a clone that worked here and not there
# would be a stage proving something else.
clone() {
  cat <<EOP
set -eu
git clone --quiet --depth 1 "https://x-access-token:\${GH_TOKEN}@github.com/$1.git" /workspace/repo
cd /workspace/repo
...
echo "PROBE go               built and tested $(go version | cut -d' ' -f3)"

# A repository asking for a toolchain this image does not carry. Routine rather
# than exotic — Go's own auto-download makes it so — and the acceptance criterion
# is that it *completes*, not that we prevent it. GOTOOLCHAIN=local would turn
# this line into a failed run that looks like the repository's fault.
mkdir -p /workspace/pinned && cd /workspace/pinned
printf 'module pinned\n\ngo 1.26.0\n\ntoolchain go1.26.0\n' > go.mod
printf 'package main\nfunc main() {}\n' > main.go
go build ./... && echo "PROBE toolchain-pin    a go.mod pinning $(go version | cut -d' ' -f3) completed, not refused"
EOP
} > "$PROBE"
...
# read-only at run time; $HOME is the emptyDir the pod can write.
