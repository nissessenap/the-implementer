...
// phase.sh writes exactly once at the end of a run. It lives here because the test
// is its only reader so far; when the orchestrator's PR builder becomes the second
// one, that is the moment it earns a file of its own.
//
...
// status "failed" and the phase in Message.
type Result struct {
